...
async function measure(path: string): Promise<{ integratedLufs: number; truePeakDbtp: number }> {
  return await new Promise((accept, reject) => {
    const child = spawn(
      'ffmpeg',
      ['-hide_banner', '-nostats', '-i', path, '-af', 'loudnorm=print_format=json', '-f', 'null', 'NUL'],
      { windowsHide: true },
    );
    let stderr = '';
...
for (const asset of manifest.assets) {
  const values = await measure(resolve(ROOT, 'public', asset.path.replace(/^\//u, '')));
  const target = asset.kind === 'voice' ? -16 : -24;
  const valid =
    Math.abs(values.integratedLufs - target) <= 1.2 &&
    values.truePeakDbtp <= -1.3;
  entries.push({ id: asset.id, locale: asset.locale, kind: asset.kind, targetLufs: target, ...values, valid });
  process.stdout.write(`${asset.locale}/${asset.id} ${values.integratedLufs.toFixed(1)} LUFS ${values.truePeakDbtp.toFixed(1)} dBTP\n`);
}
await writeFile(
...
if (invalid.length > 0) throw new Error(`Invalid loudness: ${invalid.map((entry) => `${entry.locale}/${entry.id}`).join(', ')}`);
...
await writeFile(
  ...
  `${JSON.stringify({
    ...
    assets: manifest.assets.map((asset) => {
      const measured = measuredByAsset.get(`${asset.kind}:${asset.locale}:${asset.id}`)!;
      return {
        ...asset,

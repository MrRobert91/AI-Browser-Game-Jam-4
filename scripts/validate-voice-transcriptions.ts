...
const REPORT_PATH = resolve(
  ROOT,
  'public/assets/audio/transcription-report.json',
);
const apiKey = process.env.OPENROUTER_API_KEY;
if (!apiKey) throw new Error('OPENROUTER_API_KEY is required.');
const limitRaw = process.argv
...
function similarity(expected: string, actual: string): number {
  ...
  return (
    table[left.length]![right.length]! / Math.max(1, left.length, right.length)
  );
}
...
for (const asset of manifest.assets
  ...
  if (!response.ok)
    throw new Error(
      `${asset.locale}/${asset.id}: HTTP ${response.status} ${(await response.text()).slice(0, 300)}`,
    );
  ...
  process.stdout.write(
    `${asset.locale}/${asset.id} ${(score * 100).toFixed(1)}%\n`,
  );
}
...
process.stdout.write(
  `Validated ${report.length} clips; STT cost $${totalCostUsd.toFixed(6)}\n`,
);
